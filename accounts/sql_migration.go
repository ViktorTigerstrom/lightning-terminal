...
func migrateSingleAccountToSQL(ctx context.Context,
	...
	if err != nil {
		return 0, fmt.Errorf("convert int err: %w", err)
	}
	...
	if err != nil {
		return 0, fmt.Errorf("insert err: %w, params %v", err, insertAccountParams)
	}
	...
	for hash := range account.Invoices {
		...
		if err != nil {
			return sqlId, fmt.Errorf("add invoice: %w", err)
		}
	}
	...
	for hash, paymentEntry := range account.Payments {
		...
		if err != nil {
			return sqlId, fmt.Errorf("add account payment: %w", err)
		}
	}
